...
impl KeyAccelerator {
    pub(crate) fn to_gtk(&self) -> Result<(gdk::ModifierType, u32), AcceleratorParseError> {
        let mods = modifiers_to_gdk_modifier_type(self.mods);
        let key = key_to_gdk_key(&self.key)?;
        Ok((mods, key))
    }
}
...
fn code_to_gdk_key(code: &Code) -> Result<u32, AcceleratorParseError> {
    gdk_key_from_name(code_to_gdk_key_name(code)?)
}
...
fn character_to_gdk_key(character: &str) -> Result<u32, AcceleratorParseError> {
    let character = single_character(character)?;
    Ok(*gdk::keys::Key::from_unicode(character))
}

fn gdk_key_from_name(name: &str) -> Result<u32, AcceleratorParseError> {
    let key = gdk::keys::Key::from_name(name);
    if key == gdk::keys::constants::VoidSymbol {
        Err(AcceleratorParseError::UnsupportedKey(name.to_string()))
    } else {
        Ok(*key)
    }
}

fn single_character(character: &str) -> Result<char, AcceleratorParseError> {
...
fn code_to_gdk_key_name(code: &Code) -> Result<&'static str, AcceleratorParseError> {
    use Code::*;
